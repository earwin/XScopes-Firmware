...
ISR(PORTA_INT0_vect) {
    ...
    Key=0;
    if(!testbit(in,1)) setbit(Key, KM); // Menu key
    #ifdef PORTABLE
        if(testbit(Display, flip)) {
            if(!testbit(in,4)) setbit(Key,K1);
            if(!testbit(in,3)) setbit(Key,K2);
            if(!testbit(in,2)) setbit(Key,K3);
        }
        else {
            if(!testbit(in,4)) setbit(Key,K3);
            if(!testbit(in,3)) setbit(Key,K2);
            if(!testbit(in,2)) setbit(Key,K1);
        }
    #else
        if(testbit(Display, flip)) {
            if(!testbit(in,4)) setbit(Key,K3);
...
ISR(RTC_COMP_vect) {
    if(!Key) Menu=Mdefault; // Go to default menu if no key is being pressed
}
